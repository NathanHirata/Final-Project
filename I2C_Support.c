...
void DS3231_Write_Time()
{
    char Device = 0x68;                                         // Device ID
    char Address = 0x00;                                        // Beginning Address 0
    second = dec_2_bcd(setup_second);                           // convert to BCD seconds
    minute = dec_2_bcd(setup_minute);                           // convert to BCD minutes
    hour = dec_2_bcd(setup_hour);                               // convert to BCD hours
    dow = 0x01;                                                 // convert to BCD day of week
    day = dec_2_bcd(setup_day);                                 // convert to BCD day
    month = dec_2_bcd(setup_month);                             // convert to BCD month
    year = dec_2_bcd(setup_year);                               // convert to BCD year
    I2C_Start();                                                // Start I2C protocol
    I2C_Write((Device << 1) | 0);                               // Device address Write mode
    I2C_Write(Address);                                         // Send register address
    I2C_Write(second);                                          // Write seconds
...
char control_E;
    ...
    alarm_second = alarm_second & 0x7f;                         // Mask off bit 7 seconds
    alarm_minute = alarm_minute & 0x7f;                         // Mask off bit 7 minutes
    alarm_hour   = alarm_hour   & 0x7f;                         // Mask off bit 7 hour
    alarm_date   = alarm_date   | 0x80;                         // Mask off bit 7 date
...
void DS3231_Write_Alarm_Time()
{
    ...
    alarm_hour = dec_2_bcd(setup_alarm_hour);                   // Convert info to BCD hours
    alarm_minute = dec_2_bcd(setup_alarm_minute);               // Convert info to BCD minutes
    alarm_second = dec_2_bcd(setup_alarm_second);               // Convert info to BCD seconds
    alarm_second = alarm_second & 0x7f;                         // Mask off bit 7 seconds
    alarm_minute = alarm_minute & 0x7f;                         // Mask off bit 7 minutes
    alarm_hour   = alarm_hour   & 0x7f;                         // Mask off bit 7 hours
...
void DS3231_Turn_Off_Alarm()                                    // reset RTC_ALARM_NOT
{
char Device = 0x68;
char Address_E = 0x0E;   
char Address_F = 0x0F;
...
void DS3231_Turn_On_Alarm()                                     // activate RTC_ALARM_NOT sequence
{
char Device = 0x68;
char Address_E = 0x0E;   
char Address_F = 0x0F;
