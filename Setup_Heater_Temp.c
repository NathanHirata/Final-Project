...
In_Heater_Temp_Setup = 1;
    setup_heater_set_temp = heater_set_temp;                            // Setup Heater Temp will be loaded with Heater Set Temp
    ...
    while (In_Heater_Temp_Setup == 1)
    {
         if (check_for_button_input() == 1)
        {
             ...
             {
                Do_Beep_Good();                                         // Produces high pitched beep
                if (found == Play_Pause) Do_Save_New_Heater_Temp();     // if Play/Pause button pressed, save set heater temp           
                if (found == Minus) Decrease_Heater_Temp();             // if Minus button pressed, decrement value
                if (found == Plus) Increase_Heater_Temp();              // if Plus button pressed, increment value
                if (found == EQ) Exit_Setup_Heater_Temp();              // if Equal button pressed, exit to main screen
                found = 0xff;
             }
             else
             {
                 Do_Beep_Bad();                                         // Produces low pitch beep
             }             
        }                   
    }
}
...
void Increase_Heater_Temp()
{
    ...
    if(setup_heater_set_temp>110)setup_heater_set_temp=110;             // then check if it is greater than 110. If so, limit to 110
...
{
    setup_heater_set_temp--;
    if(setup_heater_set_temp<50)setup_heater_set_temp=50;               // decrease setup_heater_set_temp by 1
...
{
    char dc_char1 = setup_heater_set_temp/100;                                                                      // Divides heater set temp by 100, Hundreds
    char dc_char2 = (setup_heater_set_temp/10)%10;                                                                  // Divides heater set temp by 10 and divide by 10 again using the remainder, Tens
    char dc_char3 = setup_heater_set_temp%10;                                                                       // Divides heater set temp by 10 and uses the remainder, Ones
    setup_heater_set_text[0] = dc_char1 + '0';                                                                      // Text storage for heater set temp hundreds
    setup_heater_set_text[1] = dc_char2 + '0';                                                                      // Text storage for heater set temp tens
    setup_heater_set_text[2] = dc_char3 + '0';                                                                      // Text storage for heater set temp ones
    drawtext(setup_data_heater_x, setup_data_heater_y ,setup_heater_set_text, ST7735_RED, ST7735_BLACK, TS_2);      // Draw text
}
