...
void Update_Screen(void)
{
    ...
    if (ALARMEN == 1)                               // display alarm mode to be on or off
    {   
    ...
    Heater_Set_Temp_Txt[0] = heater_set_temp/100  + '0';  // Heater temp 100th digit
    Heater_Set_Temp_Txt[1] = (heater_set_temp/10)%10 + '0'; // Heater temp 10th digit
    Heater_Set_Temp_Txt[2] = heater_set_temp%10 + '0';   // Heater temp ones digit
    
    
    if (HEATER == 1)                                // display heater to be on or off
    {
    ...
    if (RTC_ALARM_NOT == 0) RTC_ALARM_Txt[0] = '1'; // RTC active low 
    else RTC_ALARM_Txt[0] = '0';                    
       
    
    DC_Txt[0] = duty_cycle/100  + '0';              // Dc 100th digit
    DC_Txt[1] = (duty_cycle/10)%10 + '0';           // Dc 10th digit
    DC_Txt[2] = duty_cycle%10 + '0';                // Dc ones digit    
    
    Volt_Txt[0] = ((int) volt) + '0';               // photoS voltage ones digit
    Volt_Txt[2] = ((int)(volt*10))%10 + '0';        // photoS voltage decimal 10ths digit
    Volt_Txt[3] = ((int)(volt*100))%10 + '0';       // photoS voltage decimal 100ths digit
    
    RPM_Txt[0] = rpm/1000  + '0';                   // rpm 1000th digit
    RPM_Txt[1] = (rpm/100)%10  + '0';               // rpm 100th digit
    RPM_Txt[2] = (rpm/10)%10  + '0';                // rpm 10th digit
    RPM_Txt[3] = rpm%10  + '0';                     // rpm ones digit        
